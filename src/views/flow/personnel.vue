<template>
    <el-dialog title="请选择" width="700px" :visible.sync="dialogVisible" :close-on-click-modal="false" append-to-body>
        <el-tabs v-model="activeName" style="margin-top: -15px">
            <el-tab-pane label="部门" name="user">
                <el-row style="margin-top: 10px">
                    ...
                    <el-col :span="11" style="height: 450px; border: 1px solid #ccc; border-radius: 4px">
                        <el-table
                        ...
                        >
                            <el-table-column type="selection" width="55" align="center"></el-table-column>
                            <el-table-column property="name"></el-table-column>
                            <el-table-column property="type" align="right">
                                <template slot-scope="scope">
                                    <el-tag size="small" v-if="scope.row.type === 'company'">单位</el-tag>
                                    <el-tag size="small" v-else-if="scope.row.type === 'department'" type="success"
                                        >部门</el-tag
                                    >
                                    <el-tag size="small" v-else-if="scope.row.type === 'user'" type="info">用户</el-tag>
                                    <el-tag size="small" v-else-if="scope.row.type === 'role'" type="warning"
                                        >角色</el-tag
                                    >
                                </template>
                            </el-table-column>
            ...
            <!-- <el-tab-pane label="相对角色" name="relative"></el-tab-pane>
            <el-tab-pane label="表单操作权限" name="authority"></el-tab-pane> -->
        </el-tabs>
        <div slot="footer" style="display: flex; align-items: center">
            <div style="width: 220px; text-align: left" v-if="activeName === 'user'">
    ...
    methods: {
        ...
        handleRight() {
            if (this.activeName === 'user') {
                ...
                if (this.currentNode) {
                    if (this.currentNode.type === 'role') {
                        let flag = true
                        for (let j = 0; j < this.checkedGridData.length; j++) {
                            if (
                                this.currentNode.value == this.checkedGridData[j].value &&
                                this.checkedGridData[j].type === 'role'
                            ) {
                                flag = false
                                break
                            }
